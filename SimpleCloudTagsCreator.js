class SimpleCloudTagsCreator {
	constructor(data, minFontSize, maxFontSize, tagsPerRow = 5, className, shuffle) {
		...
		this.errors = {
			...
			tagsPerRowErrors: [ 'Expect number', 'Number cannot be less than one' ]
		};
	...
	genTagsCloud() {
		...
		for (let key of Object.keys(this.dataArr[0])) {
			...
			size =
				this.minFontSize +
				(this.dataArr[0][key].weight - minCount) * (this.maxFontSize - this.minFontSize) / spread;
	...
	applyUserStyles(size, key) {
		...
		style += `font-size: ${Math.floor(size)}px; `;
		style += `padding-left: ${this.defaultTagStyles['padding-left']}; `;
		style += `padding-right: ${this.defaultTagStyles['padding-right']}; `;
		style += '}';
		return style;
	}
}
